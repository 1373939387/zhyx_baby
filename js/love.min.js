...
$(function() {
	...
	$("#write-submit").click(function() {
		...
		$("#write-post").click(function() {
			...
			} else if (textmail.replace(/\s+/g, "") == "") {
				$("#write-mail i").text("←邮箱不能为空").fadeIn();
				$("#text-mail").focus();
				setTimeout(function() {
					$("#write-mail i").fadeOut();
				}, 3000);
			} else if (!/^[\w-]+(\.[\w-]+)*@([\w-]+\.)+(com|cn|net|org|so|biz|cc)$/.test(textmail)) {
				$("#write-mail i").text("←格式错!邮箱如1308484422@qq.com").fadeIn();
				$("#text-mail").focus();
				setTimeout(function() {
					$("#write-mail i").fadeOut();
				}, 3000);
				...
				$.post("love.php?add", {
					...
					function(data) {
						if (data['status'] == 1) {
							...
							$("#write-url").html('您的表白链接是：<a href="' + ['mail.php?mail=' + textmail + '&wz=' + data['url'] + ''] + '" target="_blank">' + data['url'] + '</a>').fadeIn();
